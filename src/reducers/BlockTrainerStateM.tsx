...
import { FBpairPosBackFS, FBpairPosFrontFS, Face, Typ } from "../lib/Defs";
...
export abstract class BlockTrainerStateM extends AbstractStateM {
  ...
  _solve(
    ...
    const name = options.updateSolutionOnly ? this.state.name : StateT.hiding;
  ...
  _updateCase(): AppState {
    let prevSol = this.state.case.desc;
    let {
      cube,
      solvers: solverNames,
  ...
  onControl(s: string): AppState {
    ...
    if (s === "#space") {
      ...
      if (state.name === StateT.inspection) {
  ...
  onMove(move: string): AppState {
    ...
    if (moves.length > 0) {
      let moveM = moves[0];
      let cube = state.cube.state.apply(moveM);
      let fb_solved = CubeUtil.is_fb_solved(cube);
      let newState: StateT;
      if (fb_solved) {
        return this._updateCase();
      } else {
        newState = StateT.solving;
      }

      if (state.name === StateT.revealed) {
      ...
      return {
        ...
        cube: {
          ...state.cube,
          state: state.cube.state.apply(moveM),
...
export class FbdrStateM extends BlockTrainerStateM {
  ...
  _get_random_fs_back() {
    ...
    for (let i = 0; i < 1000; i++) {
      if (
        this._edge_piece_in_pattern(cube, 7, this.allowed_dr) &&
  ...
  getLevelSelector() {
    return this.state.config.fbdrLevelSelector;
  }
  getRandomAnyLevel(): RandomCubeT {
    const fbOnly =
      this.state.config.fbOnlySelector.getActiveName() === "FB Last Pair Only";
    const pairSolved =
      this.state.config.fbPairSolvedSelector.getActiveName() !== "Random";
    const scrambleType =
      this.state.config.fbdrScrambleSelector.getActiveName() || "Short";
    ...
    this.allowed_pedge = this.state.config.fbdrPosSelector1.flags
      .map((value, i) => [value, i])
      .filter(([value, i]) => value)
      .map(([value, i]) => this.edgePositionMap[i]);
    this.allowed_dr = this.state.config.fbdrPosSelector3.flags
      .map((value, i) => [value, i])
      .filter(([value, i]) => value)
      .map(([value, i]) => this.edgePositionMap[i]);
...
export class FbStateM extends BlockTrainerStateM {
  ...
  _find_center_connected_edges(cube: CubieCube, is_l_only: boolean) {
    ...
    let edges = CubeUtil.stickers.filter(
      (c) =>
        c[2] === Typ.E &&
        centers.includes(c[3]) &&
  ...
  _get_random(): [CubieCube, string] {
    ...
    else if (active === "BL Solved") mask = Mask.bl_solved_mask;
    //else if (active === "BL Pair Solved")
    //    mask = Mask.bl_pair_solved_mask;
    else if (active === "Zhouheng Variant") mask = Mask.zhouheng_mask;
    ...
    cube =
      basis === "Default"
        ? cube
        : basis === "DL"
        ? CubeUtil.rebase_to_edge(cube, 5)[0]
        : CubeUtil.rebase_to_edge(cube, 9)[0]; // BL

    let solver =
      basis === "Default" ? "fb" : basis === "DL" ? "fb@dl" : "fb@bl";
  ...
  getRandomAnyLevel() {
    ...
    return {
      cube,
      solvers: solver === "min2phase" ? [] : [solver],
      ssolver: solver,
    };
  }
}
...
export class FsStateM extends BlockTrainerStateM {
  ...
  getLevelSelector() {
    return this.state.config.fsLevelSelector;
  }
  getRandomAnyLevel() {
...
export class FbssStateM extends BlockTrainerStateM {
  ...
  getLevelSelector() {
    return this.state.config.fbssLevelSelector;
  }
  getRandom() {
