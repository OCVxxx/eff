...
export class CubieCube {
  ...
  static generate_apply_partial_func_perm(
    pc: Array<PermChg>,
    mod: number,
    p: string
  ) {
  ...
  static generate_apply_func(move: Move): (c: CubieCube) => CubieCube {
    ...
    return new Function(
      ...
      `
        ...
        ${CubieCube.generate_apply_partial_func(
          move.coc,
          move.cpc,
          C_MOD,
          "co",
  ...
  _from_facelet_mapping(
    ...
    let match_pos_piece = (
      ...
      return [
        ori,
        expected.get(pos.toString())!,
        expected.get(piece.toString())!,
      ];
    };

    for (let coord_pair of fm) {
      let pos = coord_pair[0],
        piece = coord_pair[1];
      let coord: Face[][] =
        pos.length === 2
          ? custom_edges_coord
      ...
      let [newOri, newpos, newPerm] = match_pos_piece(
        pos,
        piece,
        coord,
        pos.length
  ...
  to_cstimer_cube() {
    let facelet_mapping = this._to_facelet_mapping(
      corners_coord,
      edges_coord,
      centers_coord
    );
    let cube = this._from_facelet_mapping(
      facelet_mapping,
      cstimer_corners_coord,
      cstimer_edges_coord,
      centers_coord
  ...
  is_solveable() {
    ...
    let ori_correct =
      this.co.reduce((x, y) => x + y, 0) % 3 === 0 &&
      this.eo.reduce((x, y) => x + y, 0) % 2 === 0;
  ...
  changeBasis(s: MoveSeq) {
    ...
    let transformed_mapping = s.moves.reduce(
      (mapping, move): [Face[], Face[]][] => {
...
export class Move {
  ...
  static generate_base_moves = () => {
    ...
    let dw = new Move([new Move(d), es[2]], "d");
    let dws = make_rot_set(dw);
  ...
  collapse(): MoveSeq {
    ...
    while (moves.length > 0) {
      ...
      if (newMoves.length === 0) {
        ...
        for (let m of combined.moves) newMoves.push(m);
      }
    }
...
let FaceletCube = (function () {
  ...
  let from_cubie = (cube: CubieCube, mask?: Mask): FaceletCubeT => {
    ...
    if (mask)
      return faces.map((facelet) =>
        from_cubie_partial_masked(cube, facelet, mask)
      );
...
let CubeUtil = (() => {
  ...
  let is_solved = (cube: CubieCube, mask: Mask) => {
    ...
    let c_true = co.every(
      (_, i) =>
        (cp[i] === 0 || cube.cp[i] === i) && (co[i] === 0 || cube.co[i] === 0)
    );
    if (!c_true) return false;
    let e_true = eo.every(
      (_, i) =>
        (ep[i] === 0 || cube.ep[i] === i) && (eo[i] === 0 || cube.eo[i] === 0)
    );
  ...
  let find_pairs = (function () {
    ...
    let ep_stickers: StickerExtT[][] = Array(12)
      .fill(0)
      .map((_) => Array(0));
    edge_stickers.map((s) => ep_stickers[s[0]].push(s));

    let cp_stickers: StickerExtT[][] = Array(8)
      .fill(0)
      .map((_) => Array(0));
  ...
  let is_fb_solved = (cube: CubieCube) => {
    return is_solved(cube, fb_mask);
  };
...
export class ColorScheme extends Storage {
  ...
  getColorsForOri(s: string) {
    let faces =
      (ColorScheme.valid_schemes_map.get(s) ||
        ColorScheme.valid_schemes_map.get("WG")) + "X";
    let arr: string[] = [];
