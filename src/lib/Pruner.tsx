...
let htm_rwm_fws = [
  ...
  "S",
  "S2",
]; // TODO: suppress the other S for OH mode?
...
let crossPrunerConfigGen = (is_fixed: boolean, max_depth: number): PrunerConfig => {
  const esize = Math.pow(24, 4);
  const tsize = 6 * 6;
  const size = esize * tsize;
  ...
  function encode(cube: CubieCube) {
    ...
    const enc_t = t1 * 6 + t2;
    return enc_e * 6 * 6 + enc_t;
  }

  const moves = [""];
  const solved_states = moves.map((move: string) =>
    new CubieCube().apply(move)
  );

  const moveset = cross_moves;
  return {
    size,
    encode,
    solved_states,
...
let fbdrPrunerConfigGen = (max_depth: number): PrunerConfig => {
  ...
  const solved_states = moves.map((move: Move[]) =>
    new CubieCube().apply(move)
  );
...
let crossPrunerConfig = (is_fixed: boolean) => crossPrunerConfigGen(is_fixed, 5);
...
let lpSbPrunerConfigsAuto = (lp_front: boolean) => [
  prunerFactory({
    ...
    edge: lp_front
      ? [I, I, I, I, I, I, I, S, S, I, I, S]
      : [I, I, I, I, I, I, I, S, I, S, S, I],
    center: [I, I, I, I, I, I],
    solved_states: ["id"],
...
function eolrPrunerConfig(
  ...
  const moves_ac = cartesianProduct(
    ["U'", "U"],
    ["M2"],
    ["", "U", "U'", "U2"]
  ).map((x) => x.join(" "));
  const moves_mc = cartesianProduct(
    ["M'"],
    ["U", "U'"],
    ["M2"],
    ["", "U", "U'", "U2"]
